...
LinkedList* ll_filter(LinkedList* this,int (*pFunc)(void*)){
    ...
    if(this!=NULL && pFunc!=NULL)
    {
        ...
        if(xLen>0 && filterList!= NULL)
        {
            for(i=0; i<xLen; i++)
            {
                auxPelement= ll_get(this,i); //obtengo el elemento en esa posicion
                if(pFunc(auxPelement)==1)// si a la funcion le gusta, lo agrego
                {
                    ll_add(filterList,auxPelement);//agregoel elemento
                    counter++;
                }
            }
        }
