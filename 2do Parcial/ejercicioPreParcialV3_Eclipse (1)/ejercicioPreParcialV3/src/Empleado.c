...
//int auxReturn=-1;
    ...
    {
    	buffer = (Employee*)p;
    	auxHs = buffer->horasTrabajadas;

		if(auxHs>=80 && auxHs<=120)
    	{
			hs1=auxHs*180;
			//auxReturn=0;
    	}
		else if(auxHs>120 && auxHs<=160)
		{
			hs2=(auxHs-120)*240
			//auxReturn=0;
		}
		else if(auxHs>160 && auxHs<=240)
		{
			hs3=auxHs-160)*350);
			//auxReturn=0;
		}
		sueldo=hs3+hs2+h1;
		buffer->sueldo = sueldo;
		/*criterio:
...
/*int al_funcionCriterio(void* p)
{
	// evaluo algo y deuelkvo 0: no entra o 1: entra
